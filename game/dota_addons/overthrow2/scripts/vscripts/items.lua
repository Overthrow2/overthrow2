...
function COverthrowGameMode:SpawnGold()
	...
	if throwCoin2 and RandomInt( 1, 100 ) > 95 then
...
function COverthrowGameMode:SpecialItemAdd( event )
	...
	if self.nNextSpawnItemNumber >= 3 then
	...
	GameRules:SendCustomMessage( owner:GetName() .. " picked up the treasure: " .. spawnedItem:GetName(), DOTA_TEAM_NOTEAM, 1)
